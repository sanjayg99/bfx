...
repo = gh.repository('bfxcrypto', 'bfx')
...
def check_assets():
    ...
    body = []
    win_icon        = '<img src="https://raw.githubusercontent.com/bfxcrypto/bfx/master/doc/img/windows-10-100.png" alt="Windows Icon by icons8 from https://icons8.com/" height="48px"/>'
    mac_icon        = '<img src="https://raw.githubusercontent.com/bfxcrypto/bfx/master/doc/img/apple-100.png" alt="Apple Icon by icons8 from https://icons8.com/" height="48px"/>'
    lin_icon        = '<img src="https://raw.githubusercontent.com/bfxcrypto/bfx/master/doc/img/linux-100.png" alt="Linux Icon by icons8 from https://icons8.com/" height="48px"/>'
    rpi_icon        = '<img src="https://raw.githubusercontent.com/bfxcrypto/bfx/master/doc/img/raspberry-pi-100.png" alt="Raspberry Pi Icon by icons8 from https://icons8.com/" height="48px"/>'
    docker_icon     = '<img src="https://raw.githubusercontent.com/bfxcrypto/bfx/master/doc/img/docker-100.png" alt="Docker Icon by icons8 from https://icons8.com/" height="48px"/>'
    docker_rpi_icon = '<img src="https://raw.githubusercontent.com/bfxcrypto/bfx/master/doc/img/docker-rpi-100.png" alt="Docker & Raspberry Pi Icon by icons8 from https://icons8.com/" height="48px"/>'
    ...
    for x in assets:
        ...
        global lin_d_str
        if ("bfxd---ubuntu16.04" in x.name and len(lin_d_str) == 0):
            x.sha256 = download_and_checksum(x.browser_download_url)
            if len(x.sha256) == 64:
        ...
        global rpi_d_str
        if ("bfxd---RPi-raspbian" in x.name and len(rpi_d_str) == 0):
            x.sha256 = download_and_checksum(x.browser_download_url)
            if len(x.sha256) == 64:
                rpi_d_str = '| ' + rpi_icon + ' | [Download ' + rel.tag_name + '<br/>For Raspberry Pi](' + x.browser_download_url + ') | `' + x.sha256 + '` |'
                # print(rpi_d_str)

        # post docker bfxd build once regular bfxd build is done building, since we cannot easily tell when the docker builds are done
        global docker_d_str
        if ("bfxd---ubuntu16.04" in x.name and len(docker_d_str) == 0):
            x.sha256 = download_and_checksum(x.browser_download_url)
            if len(x.sha256) == 64:
                docker_d_str = '| ' + docker_icon + ' | [Download ' + rel.tag_name + '<br/>For Docker](' + 'https://hub.docker.com/r/bfxteam/bfxd/builds' + ') | `' + 'N/A' + '` |'
                # print(docker_d_str)
    ...
    body.append('')
    body.append('')
    body.append('## bfxd (Server Node. Command Line Only)')
    body.append('| System | Download | Sha256 Checksum |')
    body.append('|:---:|:---:|:---|')
    if len(lin_d_str) > 0:
