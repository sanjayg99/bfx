#include "issuenewbfxttokendialog.h"
...
void IssueNewBFXTTokenDialog::createWidgets()
{
    setWindowTitle("Issue a new BFXT token");
    ...
    metadataDialog        = new BFXTCreateMetadataDialog(this);
    ...
    costLabel->setText("Issuing a token will cost " +
                       QString::number(BFXTTransaction::IssuanceFee / COIN) +
                       " BFX. BFXT outputs cannot be used.");
    ...
    tokenSymbolValidator = new BFXTTokenSymbolValidator(*this, this);
    ...
    amountLineEdit->setValidator(new BFXTTokenAmountValidator(this));

    connect(this->clearButton, &QPushButton::clicked, this, &IssueNewBFXTTokenDialog::slot_clearData);
    connect(this->changeAddressLineEdit, &QLineEdit::textChanged, this,
            &IssueNewBFXTTokenDialog::slot_modifyChangeAddressColor);
    connect(this->targetAddressLineEdit, &QLineEdit::textChanged, this,
            &IssueNewBFXTTokenDialog::slot_modifyTargetAddressColor);
    connect(this->changeAddressCheckbox, &QCheckBox::toggled, this,
            &IssueNewBFXTTokenDialog::slot_changeAddressCheckboxToggled);
    connect(this->cancelButton, &QPushButton::clicked, this, &IssueNewBFXTTokenDialog::hide);
    connect(this->editMetadataButton, &QPushButton::clicked, this->metadataDialog,
            &BFXTCreateMetadataDialog::show);
    connect(this->issueButton, &QPushButton::clicked, this, &IssueNewBFXTTokenDialog::slot_doIssueToken);
    connect(this->iconUrlLineEdit, &QLineEdit::textChanged, this,
            &IssueNewBFXTTokenDialog::slot_iconUrlChanged);
    connect(this->coinControlButton, &QPushButton::clicked, this,
            &IssueNewBFXTTokenDialog::slot_coinControlButtonClicked);
...
IssueNewBFXTTokenDialog::IssueNewBFXTTokenDialog(QWidget* parent) : QDialog(parent) { createWidgets(); }

void IssueNewBFXTTokenDialog::clearData()
{
...
void IssueNewBFXTTokenDialog::validateInput() const
{
...
void IssueNewBFXTTokenDialog::setWalletModel(WalletModel* WalletModelPtr)
{
    walletModel = WalletModelPtr;
}

json_spirit::Value IssueNewBFXTTokenDialog::getIssuanceMetadata() const
{
    validateInput();
...
void IssueNewBFXTTokenDialog::slot_clearData()
{
...
void IssueNewBFXTTokenDialog::slot_modifyChangeAddressColor()
{
...
void IssueNewBFXTTokenDialog::slot_modifyTargetAddressColor()
{
    if (CBitcoinAddress(targetAddressLineEdit->text().toStdString()).IsValid()) {
...
void IssueNewBFXTTokenDialog::slot_changeAddressCheckboxToggled(bool checked)
{
...
void IssueNewBFXTTokenDialog::slot_doIssueToken()
{
    try {
        validateInput();
        ...
        RawBFXTMetadataBeforeSend metadata(json_spirit::write(metadataObj));

        if (pwalletMain == nullptr) {
            throw std::runtime_error("The wallet pointer is null. Failed to create transaction.");
        }

        const int64_t minAmount = 2 * MIN_TX_FEE + BFXTTransaction::IssuanceFee;
        ...
        BFXTInt     amount(amountLineEdit->text().toStdString());
        std::string tokenSymbol = tokenSymbolLineEdit->text().toStdString();

        BFXTSendTokensOneRecipientData bfxtrecipient;
        bfxtrecipient.amount = amount;
        bfxtrecipient.destination =
            CBitcoinAddress(targetAddressLineEdit->text().toStdString()).ToString();
        bfxtrecipient.tokenId = BFXTSendTxData::TO_ISSUE_TOKEN_ID;

        std::vector<BFXTSendTokensOneRecipientData> bfxtrecipients{bfxtrecipient};
        ...
        // make sure the amount is correct AND make sure that all inputs are non-BFXT
        if (takeInputsFromCoinControl) {
            ...
            for (const COutPoint o : inputs) {
                ...
                bool txIsBFXT = BFXTTransaction::IsTxBFXT(&tx);
                ...
                if (txIsBFXT) {
                    ...
                    try {
                        ...
                        if (bfxttx.getTxOut(o.n).tokenCount() > 0) {
                            QMessageBox::warning(
                                this, "Error",
                                "One or more of the inputs chosen in coin control is an BFXT output. "
                                "\n\nFor "
                                "issuance, you cannot choose BFXT outputs. Please choose outputs that "
                                "contain only NEBLs");
                            return;
                        }
        ...
        tokenSelector.issueBFXTToken(IssueTokenData(amount, tokenSymbol, metadata.metadata));
        tokenSelector.selectBFXTTokens(bfxtwallet, inputs, bfxtrecipients, !takeInputsFromCoinControl);
...
void IssueNewBFXTTokenDialog::slot_iconUrlChanged(const QString& url)
{
...
void IssueNewBFXTTokenDialog::slot_coinControlButtonClicked()
{
...
QValidator::State BFXTTokenSymbolValidator::validate(QString& input, int&) const
{
    if (input.isEmpty()) {
    ...
    if (BFXTScript::IsBFXTTokenSymbolValid(input.toStdString())) {
...
void BFXTTokenSymbolValidator::setAlreadyIssuedTokenSymbols(
    const std::unordered_set<std::string>& tokenSymbols)
{
    alreadyIssuedTokenSymbols = tokenSymbols;
}

bool BFXTTokenSymbolValidator::tokenWithSymbolAlreadyIssued(const std::string& tokenSymbol)
{
    return alreadyIssuedTokenSymbols.find(tokenSymbol) != alreadyIssuedTokenSymbols.cend();
}

BFXTTokenSymbolValidator::BFXTTokenSymbolValidator(IssueNewBFXTTokenDialog& isseNewBFXTDialog,
                                                   QObject*                 parent)
    : QValidator(parent), dialog(isseNewBFXTDialog)
{
}

BFXTTokenAmountValidator::BFXTTokenAmountValidator(QObject* parent) : QValidator(parent) {}

QValidator::State BFXTTokenAmountValidator::validate(QString& input, int&) const
{
    if (input.isEmpty()) {
        return State::Intermediate;
    }
    ...
    BFXTInt amount(input.toStdString());
    if (amount <= 0) {
        return State::Invalid;
    }
    if (amount > BFXTMaxAmount) {
