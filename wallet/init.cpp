...
// Start
//
#if !defined(QT_GUI) && !defined(BFX_UNITTESTS)
bool AppInit(int argc, char* argv[])
{
    ...
    try {
        ...
        if (mapArgs.exists("-?") || mapArgs.exists("--help")) {
            ...
            std::string strUsage =
                _("bfx version") + " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
                "  bfxd [options]                     " + "\n" +
                "  bfxd [options] <command> [params]  " + _("Send command to -server or bfxd") +
                "\n" + "  bfxd [options] help                " + _("List commands") + "\n" +
                "  bfxd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bfx:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("bfx"),
...
std::string HelpMessage()
{
    ...
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: bfx.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: bfxd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. bfx is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(
            _("Cannot obtain a lock on data directory %s.  bfx is probably already running."),
            strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "bfx server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName)) {
        ...
        if (r == CDBEnv::RECOVER_OK) {
            string msg = strprintf(_("Warning: wallet.dat corrupt, data salvaged!"
                                   ...
                                   strDataDir.c_str());
            uiInterface.ThreadSafeMessageBox(msg, _("bfx"),
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of bfx") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE) {
            strErrors << _("Wallet needed to be rewritten: restart bfx to complete") << "\n";
