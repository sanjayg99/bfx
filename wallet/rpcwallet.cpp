...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error("getnewaddress [account]\n"
                            "Returns a new bfx address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current bfx address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error("setaccount <bfxaddress> <account>\n"
                            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid bfx address");
...
{
    if (fHelp || params.size() != 1)
        throw runtime_error("getaccount <bfxaddress>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error("sendtoaddress <bfxaddress> <amount> [comment] [comment-to]\n"
...
Value udtobfxaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "udtobfxaddress <unstoppable domain address>\n"
            "Returns the bfx address associated with the provided unstoppable domain");
    ...
    auto neblAddress = GetBFXAddressFromUDAddress(strUDDomain);
    if (!neblAddress) {
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error("signmessage <bfxaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error("verifymessage <bfxaddress> <signature> <message>\n"
...
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error("getreceivedbyaddress <bfxaddress> [minconf=1]\n"
                            "Returns the total amount received by <bfxaddress> in transactions with "
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    for (const Pair& s : sendTo) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid bfx address: ") + s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
                     ...
                     "each key is a bfx address or hex-encoded public key\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; bfx server stopping, restart to run with encrypted wallet.  The "
...
{
    if (fHelp || params.size() != 1)
        throw runtime_error("validateaddress <bfxaddress>\n"
                            "Return information about <bfxaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error("validatepubkey <bfxpubkey>\n"
                            "Return information about <bfxpubkey>.");
