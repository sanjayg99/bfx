...
void ParseParameters(int argc, const char* const argv[])
{
...

    {
        ...
        mapArgs.set("-addnode", "157.245.15.154");
        std::vector<std::string> nodes;
        mapMultiArgs.get("-addnode", nodes);
        nodes.push_back("157.245.15.162");
        nodes.push_back("157.245.15.154");
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bfx
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bfx
    // Mac: ~/Library/Application Support/bfx
    // Unix: ~/.bfx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bfx";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bfx.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bfxd.pid"));
                ...
                if (!fMatch) {
                    ...
                    string strMessage =
                        _("Warning: Please check that your computer's date and time are correct! If "
                          "your clock is wrong bfx will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage + " ", string("bfx"),
