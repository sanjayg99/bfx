...
std::string BFXTScript_Issuance::__getAggregAndLockStatusTokenIDHexValue() const
{
...
BFXTScript_Issuance::BFXTScript_Issuance() {}

std::string BFXTScript_Issuance::getHexMetadata() const { return boost::algorithm::hex(metadata); }

std::string BFXTScript_Issuance::getRawMetadata() const { return metadata; }

std::string BFXTScript_Issuance::getInflatedMetadata() const
{
    if (!metadata.empty())
...
int BFXTScript_Issuance::getDivisibility() const { return issuanceFlags.divisibility; }

bool BFXTScript_Issuance::isLocked() const { return issuanceFlags.locked; }

BFXTScript::IssuanceFlags::AggregationPolicy BFXTScript_Issuance::getAggregationPolicy() const
{
    return issuanceFlags.aggregationPolicy;
}

std::string BFXTScript_Issuance::getAggregationPolicyStr() const
{
    if (issuanceFlags.aggregationPolicy == BFXTScript::IssuanceFlags::AggregationPolicy_Aggregatable) {
        return IssuanceFlags::AggregationPolicy_Aggregatable_Str;
    } else if (issuanceFlags.aggregationPolicy ==
               BFXTScript::IssuanceFlags::AggregationPolicy_Aggregatable) {
...
std::string BFXTScript_Issuance::getTokenSymbol() const { return tokenSymbol; }

BFXTInt BFXTScript_Issuance::getAmount() const { return amount; }

unsigned BFXTScript_Issuance::getTransferInstructionsCount() const
{
    return transferInstructions.size();
}

BFXTScript::TransferInstruction BFXTScript_Issuance::getTransferInstruction(unsigned index) const
{
    return transferInstructions[index];
}

std::vector<BFXTScript::TransferInstruction> BFXTScript_Issuance::getTransferInstructions() const
{
    return transferInstructions;
}

std::shared_ptr<BFXTScript_Issuance>
BFXTScript_Issuance::ParseIssuancePostHeaderData(std::string ScriptBin, std::string OpCodeBin)
{
    std::shared_ptr<BFXTScript_Issuance> result = std::make_shared<BFXTScript_Issuance>();
...
std::shared_ptr<BFXTScript_Issuance>
BFXTScript_Issuance::ParseBFXTv3IssuancePostHeaderData(std::string ScriptBin)
{
    std::shared_ptr<BFXTScript_Issuance> result = std::make_shared<BFXTScript_Issuance>();
...
std::string BFXTScript_Issuance::getTokenID(std::string input0txid, unsigned int input0index) const
{
...
std::string BFXTScript_Issuance::calculateScriptBin() const
{
    ...
    if (protocolVersion == 1) {
        ...
        result += metadata;
        result += unhex(NumberToHexBFXTAmount(amount));
...
std::shared_ptr<BFXTScript_Issuance> BFXTScript_Issuance::CreateScript(
    const std::string& Symbol, BFXTInt amount,
...
{
    std::shared_ptr<BFXTScript_Issuance> script = std::make_shared<BFXTScript_Issuance>();
...
std::string BFXTScript_Issuance::Create_OpCodeFromMetadata(const std::string& metadata)
{
...
std::string BFXTScript_Issuance::Create_ProcessTokenSymbol(const std::string& symbol)
{
    if (symbol.size() > 5) {
...
std::set<unsigned int> BFXTScript_Issuance::getBFXTOutputIndices() const
{
